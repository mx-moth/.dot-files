...
# If IS_REMOTE_HOST=1, LC_ORIGIN_HOST will contain the hostname of the computer
# you are SSHing from. Used in vim to display the current hostname in the top
# left, and in PS1 to display the origin host
if [ -z "$LC_ORIGIN_HOST" ] || [ "$LC_ORIGIN_HOST" == "$HOSTNAME" ] ; then
	export LC_ORIGIN_HOST="$HOSTNAME"
else
