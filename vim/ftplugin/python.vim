...
function! PythonFoldText()
  ...
  let w = winwidth(0) - &foldcolumn
  if &number
    let w = w - float2nr(ceil(log10(line('$')))) - 1
  endif
  ...
  let sizenote = ' ' . size . ' lines '

  let textlen = strlen(line) + strlen(sizenote)

  let padpattern = '═'
  let padding = repeat(padpattern, w - textlen - 1)

  " return padding
  return line . padding . sizenote
endfunction
