...
sub not_default
{
    ...
    foreach my $option (@_)
    {
        return 1 if ( weechat::config_color($option) ne "default" )
    }
    return 0;
}
...
sub build_buffers
{
    ...
    if ( ($position eq "top") or ($position eq "bottom") ) {
        $orientation = "horizontal";
    }

    my $indenting = weechat::config_integer( $options{"indenting"} );
...
#            and $buffer->{"type"} eq "channel"
    ...
    foreach my $key (sort keys %sorted_buffers)
    {
        ...
        my $color = "";
        my $bg = "";
        my $b_name = $buffer->{"name"};
        my $b_number = $buffer->{"number"};
        my $b_pointer = $buffer->{"pointer"};
        my $b_current = $buffer->{"current_buffer"};
        my $b_type = weechat::buffer_get_string($buffer->{"pointer"}, "localvar_type");

        $color = weechat::config_color( $options{"color_default_fg"} );
        $bg = weechat::config_color( $options{"color_default_bg"} );

        if ( ($b_type eq "private") and not_default( ($options{"queries_default_fg"}, $options{"queries_default_bg"}) ) )
        {
        ...
        if ( ($b_type ne "channel" ) and ( $b_type ne "private") )
        {
        ...
        if (grep {$_ eq $b_name} @whitelist_buffers)
        {
        ...
        if (exists $hotlist{$b_pointer})
        {
        delete $buffers_timer{$b_pointer};
            # check if buffer is in whitelist buffer
            if (grep {$_ eq $b_name} @whitelist_buffers) {
                $bg = weechat::config_color( $options{"color_whitelist_".$hotlist_level{$hotlist{$b_pointer}}."_bg"} );
                $color = weechat::config_color( $options{"color_whitelist_".$hotlist_level{$hotlist{$b_pointer}}."_fg"} );
            } elsif ( $b_type eq "private" ) {
                ...
                if ( not_default( (
                  ...
                  if ( ($hotlist{$b_pointer}) == 2 ) {
                      $bg = weechat::config_color( $options{"queries_message_bg"} );
                      $color = weechat::config_color( $options{"queries_message_fg"} );
                  } elsif ( ($hotlist{$b_pointer}) == 3 ) {
                      $bg = weechat::config_color( $options{"queries_highlight_bg"} );
                      $color = weechat::config_color( $options{"queries_highlight_fg"} );
                  }
                } else {
                      $bg = weechat::config_color( $options{"color_hotlist_".$hotlist_level{$hotlist{$b_pointer}}."_bg"} );
                      $color = weechat::config_color( $options{"color_hotlist_".$hotlist_level{$hotlist{$b_pointer}}."_fg"}  );
                }
            } else {
                      $bg = weechat::config_color( $options{"color_hotlist_".$hotlist_level{$hotlist{$b_pointer}}."_bg"} );
                      $color = weechat::config_color( $options{"color_hotlist_".$hotlist_level{$hotlist{$b_pointer}}."_fg"}  );
        ...
        if ($old_number ne $b_number) {
            $str .= $b_number."│";
        } else {
            $str .= "&│"
        }

        if (( $indenting ne 0 ) && ($orientation eq "vertical"))
        {
            if (($b_type eq "channel") || ($b_type eq "private"))
            {
                if ( $indenting eq 1 )
                {
                    $str .= "  ";
                }
                elsif ( ($indenting eq 2) and (weechat::config_integer($options{"indenting_number"}) eq 0) )        #under_name
                {
                    ...
                    {
                      $str .= ( (" " x ( $max_number_digits - length($b_number) ))." " );
                    }
                }
            }
        }
        ...
        if ( $b_type eq "server" and weechat::config_boolean($options{"show_lag"}) eq 1)
        {
